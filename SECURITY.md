...
Email **nolan@atlanta-robotics.org** with the subject `Security report: african-ai-cloud`. Include:
...
Never commit credentials, private keys, tokens, Terraform state, production configuration, or site-specific sensitive network details. Use environment variables or an approved secrets manager. If a secret is committed, revoke or rotate it immediately; deleting the current file does not remove it from Git history.
